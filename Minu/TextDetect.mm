...
+(NSMutableArray *)getRegionRect : (ExtremeRegionStat *) stat{
    NSMutableArray *rectRegion = [NSMutableArray arrayWithCapacity:20];
    //_regions.push_back([stat getRegion]);
    vector<ERStat> r = [stat getRegion];
    unsigned long rsize = r.size();
    for(int j = 0; j< rsize; j++){
        //printf("Region Add: %d\n",j);
        NSArray *rectArray = [NSArray arrayWithObjects:[NSNumber numberWithInt: r[j].rect.x],[NSNumber numberWithInt: r[j].rect.y],[NSNumber numberWithInt: r[j].rect.width],[NSNumber numberWithInt: r[j].rect.height], nil];
        [rectRegion addObject:rectArray];
    }
    return rectRegion;
}

+(NSMutableArray*)getGroup : (CImage*)image WithRegions: (NSArray *)regions{
    ...
    erGrouping(cvImg, [image getCVChannels] , _regions, nm_region_groups, nm_boxes,ERGROUPING_ORIENTATION_HORIZ);
    NSMutableArray *box = [NSMutableArray arrayWithCapacity:20];
    for(int i = 0; i< nm_boxes.size(); i++){
        [box addObject:[NSValue valueWithCGRect:CGRectMake(nm_boxes[i].x, nm_boxes[i].y, nm_boxes[i].width, nm_boxes[i].height)]];
    }
    return box;
}
